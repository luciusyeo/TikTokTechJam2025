      ...
      try {
        ...
        try {
          ...
          if (recommendations.recommendations.length > 0) {
            ...
            console.log('Video IDs from recommendations:', videoIds);
    ...
    try {
      ...
      if (currentUserVector && currentUserVector.length > 0) {
        try {
          ...
          const recommendations = await fetchRecommendations(currentUserVector, 10);

          console.log('Recommendations from API for more videos:', recommendations.recommendations);

          if (recommendations.recommendations.length > 0) {
            const videoIds = recommendations.recommendations.map(rec => rec.id);
            console.log('[feed] Video IDs:', videoIds);

            const newVideoIds = videoIds.filter(id => !videos.find(v => v.id === String(id)));
            ...
            if (newVideoIds.length > 0) {
              ...
              console.log('All recommended videos already loaded, no new videos.');
            }
          } else {
      ...
      const uniqueVideos = moreVideos.filter(newVideo => !videos.some(existingVideo => existingVideo.id === newVideo.id));

      console.log("Total videos after adding new ones:", [...videos, ...uniqueVideos].length);
  ...
  const onViewableItemsChanged = useCallback(({ viewableItems }: any) => {
    if (viewableItems.length > 0) {
      ...
      if (activeIndex === videos.length - 1 && !isLoadingMore) {
        console.log(`User reached the last video: ${activeIndex}, loading more...`);
        loadMoreVideos();
      } else {
